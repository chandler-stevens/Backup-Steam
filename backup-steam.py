...
def backup():
    ...
    manifests = "C:/Steam/steamapps"
    source = "C:/Steam/steamapps/common"
    destination = "D:/Steam-Games-Backup"

    # Check paths
    if not path.exists(manifests):
        if GUI:
            progress.destroy()
        write_log("FATAL ERROR: Could not find app manifests path:\n"+manifests+"\n")
        exit(1)
    ...
    for acf in next(walk(manifests))[2]:
        if search("appmanifest_[0-9]+\.acf", acf):
    ...
    if len(src_games) <= 0:
        ask_user = True
        print_and_log("WARNING: <No games are installed currently>")
    ...
    dest_games = []
    stored_games = 0
    print_and_log("\nList of Stored/Uninstalled Games:", True)
    ...
    if stored_games <= 0:
        print_and_log("<No games are stored/uninstalled currently>", True)
    ...
    for game in src_games:
        ...
        # If game not in D, then copy to D
        ...
        # otherwise:
            ...
            for folder in folders:
                # if folder not in C, then delete from D
                ...
                # otherwise:
                    ...
                    for file in files:
                        clear_cache()
                        # if file not in C, then delete from D
            ...
            if GUI:
                ...
                progress.update()
            for folder in folders:
                # If folder not in D, then copy to D
